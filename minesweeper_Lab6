...
int uncover_blank_cell(int row, int col) {
	...
	for (i = 0; i < 8; i++) {
		...
		bool isValid = (rows[i] >= 0 && rows[i] < MAX)

		&& (columns[i] >= 0 && columns[i] < MAX);
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		while (MinesNumber != 0) {			// when nMines becomes 0 you will win the game
